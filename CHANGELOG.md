...
- `deep-security-review`: `SKILL.md` is a router (commands, load, Done → phase READ). Domain checklists are a **floor**. Hunter **load** is 1 domain + 0 or 1 shape; after those files, the hunter works in the code. Phase files open when the current criterion is met.
