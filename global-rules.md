...
- **_Tight_ ladder** (tactical work — stop at the first rung that holds): (1) must this be built at all? (YAGNI) (2) reuse in-repo helper/pattern (3) standard library (4) native platform feature (5) installed dependency — read that version’s docs/types first (6) can it be one line? (7) only then minimum new code. Add a dependency only when it reduces total complexity and risk. Prefer deletion over addition, boring over clever, fewest files. When two options are the same size, pick the edge-case-correct one — less code, not the flimsier algorithm.

## While changing code

- **Tight for tactical work:** climb the ladder; add no abstraction, configuration, boilerplate, or indirection unless the user explicitly requested it.
