...
2. Apply the **minimal local fix** that closes the issue and respects Phase 1 what-must-not-change (from the report context). Scope of the fix = finding path + callers you must touch; no drive-by simplification outside the finding.
3. Redact secret values in commits, comments, and logs; rotate out-of-band if the finding was a leaked secret.
4. Do not relax auth or validation to make checks pass.
5. Inspect the fix's own diff and re-read touched callers.
6. Run project checks when they exist (test / lint / typecheck / format).
7. Run the **fix acceptance gate** before marking the finding closed.
