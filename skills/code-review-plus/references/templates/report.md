...
1. **Skeleton fill** — emit Template headings in order, only those with a body. Always emit `## Review Summary`, `### Findings Overview`, `### Verification`, and `### Verdict`. Severity detail sections expand findings; they do not replace Findings Overview.
...
5. **Verification blocks** — emit Author claimed only when the author claimed something. Emit the Manual / UI line only when the review source has a UI.
