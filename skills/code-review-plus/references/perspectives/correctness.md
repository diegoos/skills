...
Confirm the framework's real return shape (throw vs `{ data, error }`) before asserting a break. Note `regression_risk` for callers and error contracts. Suggested fix preserves error paths.
