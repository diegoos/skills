...
Prompts are not a security boundary. Hunt rules (*boundary-crossing*, confused deputy) live in the domain — this file holds stack probes.
...
- Untrusted prompt/RAG that steers a tool into a backend sink with no HTTP route still counts — isolated "low severity utility" is not a drop
...
- Can one user use the model to affect another user or tenant (*boundary-crossing*)?
- Can the model invoke a tool that reaches a sink the caller cannot reach via the normal API?
